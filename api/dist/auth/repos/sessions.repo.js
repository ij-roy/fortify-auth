...
let SessionsRepo = class SessionsRepo {
    ...
    async createSession(userId, userAgent) {
        const r = await this.db.query(`INSERT INTO auth_sessions (user_id, user_agent)
       VALUES ($1,$2)
       RETURNING id, user_id, created_at, revoked_at`, [userId, userAgent ?? null]);
        return r.rows[0];
    }
    async revokeSession(sessionId) {
        await this.db.query(`UPDATE auth_sessions
       SET revoked_at=now()
